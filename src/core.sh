...
get_ip() {
    ...
    # 如果存在自定义 IP 配置文件，直接读取，跳过自动探测
    local _custom_ip_file=$is_core_dir/custom_ip
    if [[ -f $_custom_ip_file ]]; then
        ip=$(cat $_custom_ip_file | tr -d '[:space:]')
        [[ $ip ]] && return
    fi
...
change() {
    ...
    13)
        ...
        cp -f $is_conf_dir/$is_config_file $is_tmp_json
        # 替换 short_id 数组中的值
        sed -i "s/short_id:\[\"[^\"]*\"\]/short_id:[\"$is_new_short_id\"]/" $is_tmp_json
        $is_core_bin check -c $is_tmp_json &>/dev/null
        [[ $? != 0 ]] && {
            rm -f $is_tmp_json
            err "Short ID 格式验证失败."
        }
        mv -f $is_tmp_json $is_conf_dir/$is_config_file
        short_id=$is_new_short_id
...
get() {
    ...
    info)
        ...
        if [[ $is_config_file ]]; then
            ...
            # 单独提取 short_id（避免污染主 jq 表达式的字段顺序）
            is_short_id=$(jq -r '.inbounds[0].tls.reality.short_id[0] // empty' <<<$is_json_str)
...
main() {
    ...
    ip)
        if [[ $2 == 'set' && $3 ]]; then
            echo "$3" > $is_core_dir/custom_ip
            _green "\n已设置自定义 IP: $3 (保存至 $is_core_dir/custom_ip)\n"
        elif [[ $2 == 'clear' || $2 == 'del' ]]; then
            rm -f $is_core_dir/custom_ip
            _green "\n已清除自定义 IP, 将恢复自动探测.\n"
        else
            get_ip
            msg $ip
            [[ -f $is_core_dir/custom_ip ]] && _yellow "(来自自定义配置文件: $is_core_dir/custom_ip)"
        fi
        ;;
    in | import)
        load import.sh
